...
void do_disconnect(struct mosquitto_db *db, struct mosquitto *context)
{
	...
	send_onoffline_status(db, context->id, NOTICE_TYPE_OFFLINE);
#ifdef WITH_WEBSOCKETS
	if(context->wsi){
...
void send_onoffline_status(struct mosquitto_db *db, char* client_id, int type)
{
	...
	_mosquitto_log_printf(NULL, MOSQ_LOG_DEBUG, "[send_onoffline_status] msg:%s", buf);
